...
function renderMalla() {
  ...
  ramos.forEach(ramo => {
    ...
    div.className = "ramo";
    if (!ramo.requisitos.length || ramo.requisitos.every(r => ramos.find(x => x.nombre === r)?.estado === "aprobado")) {
      ramo.estado = ramo.estado || "activo";
    } else {
      ramo.estado = ramo.estado || "bloqueado";
    }
